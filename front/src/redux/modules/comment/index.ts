...
import { Comment, GetCommentFB, DelCommentFB,  AddCommentFB } from "./types";
...
const delComment = createAction(DEL_COMMENT, (post_id, comment_id) => ({
  post_id,
  comment_id,
}));
...
const addCommentFB =
  ...
  async (dispatch, getState) => {
    ...
    try {
      ...
      //noti
      const _noti_item = await realtime
...
const delCommentFB =
  ({ comment_id = null, post_id = null }: DelCommentFB) =>
  async (dispatch) => {
...
export default handleActions(
  {
    ...
    [DEL_COMMENT]: (state, action) =>
      produce(state, (draft) => {
        const idx = draft.list[action.payload.post_id].findIndex(
          (d) => d.id === action.payload.comment_id
        );
